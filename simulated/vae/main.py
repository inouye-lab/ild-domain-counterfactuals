...
def main():
    ...
    parser.add_argument('--gt', type=str, default='inv', choices=['inv', 'pseudo_inv','hardcode'])
    ...
    parser.add_argument('--lamb_kld', type=float, default=1)  # this is the kld lambda at the start of training
    ...
    if args.wandb or args.sweep:
        wandb.init(project=f"[{args.project_prefix}]CF-SimVae-DEBUG" if not args.sweep else None,
    ...
    train_set = SimulatedFlowDataset(num_samples=args.num_samples, n_domains=args.n_domains,
                                 latent_dim=args.obs_dim, int_set=args.int_set,
                                 model_seed=args.seed, noise_seed=args.seed + 100,
                                 device=args.device, relu_slope=args.leaky_relu_slope,bias_scale=args.bias_scale)
    ...
    val_set = SimulatedFlowDataset(num_samples=int(1000), n_domains=args.n_domains,
                               ...
                               normalization_latent_dict=train_set.normalization_latent_dict,bias_scale=args.bias_scale)
